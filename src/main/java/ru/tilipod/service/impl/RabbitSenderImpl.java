...
public class RabbitSenderImpl implements RabbitSender {
    ...
    private Message createMessage(ParserResultMessage message) throws JsonProcessingException {
    ...
    @Override
    public void sendErrorToScheduler(ParserResultErrorMessage message) {
        try {
            rabbitTemplate.send(ExchangeNames.ERROR, RoutingKeys.PARSER_RESULT_KEY, createMessage(message));
        } catch (JsonProcessingException e1) {
            log.error("Ошибка сериализации {}: {}", message.getClass().getSimpleName(), e1.getMessage());
            rabbitTemplate.send(ExchangeNames.ERROR, RoutingKeys.PARSER_RESULT_KEY,
    ...
    @Override
    public void sendSuccessToScheduler(ParserResultSuccessMessage message) {
        try {
            rabbitTemplate.send(ExchangeNames.SUCCESS, RoutingKeys.PARSER_RESULT_KEY, createMessage(message));
        } catch (JsonProcessingException e1) {
            log.error("Ошибка сериализации {}: {}", message.getClass().getSimpleName(), e1.getMessage());
            rabbitTemplate.send(ExchangeNames.ERROR, RoutingKeys.PARSER_RESULT_KEY,
                    MessageBuilder.withBody(String.format(JSON_PROCESSING_ERROR_MESSAGE, message.getTaskId(), e1.getMessage())
